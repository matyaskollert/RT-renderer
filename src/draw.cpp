...
bool DrawUnvisitedInDifferentColor = true;
bool DrawIntersectedBVHs = true;
bool DrawHitTriangle = true;
bool DrawInterpolatedNormal = true;
bool DrawVertexNormals = true;
bool DrawInterpolatedNormalColor = true;
bool DrawShadingRay = true;
bool DrawShadowRay = true;
bool DrawDepthOfField = true;
...
{
    if (!enableDebugDraw || HideDrawings)
        return;
...
void drawDebugAABB(const AxisAlignedBox& box, DrawMode drawMode, const glm::vec3& color, float transparency)
{
    if (!enableDebugDraw || HideDrawings)
        return;

    drawAABB(box, drawMode, color, transparency);
}