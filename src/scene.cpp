...
Scene loadScenePrebuilt(SceneType type, const std::filesystem::path& dataDir)
{
    Scene scene;
    scene.type = type;
    scene.environmentMapLeft = std::make_shared<Image>(dataDir / "posx.jpg");
    scene.environmentMapRight = std::make_shared<Image>(dataDir / "negx.jpg");
    scene.environmentMapTop = std::make_shared<Image>(dataDir / "posy.jpg");
    scene.environmentMapBottom = std::make_shared<Image>(dataDir / "negy.jpg");
    scene.environmentMapBack = std::make_shared<Image>(dataDir / "negz.jpg");
    scene.environmentMapFront = std::make_shared<Image>(dataDir / "posz.jpg");
    switch (type) {
    case SingleTriangle: {
    ...
    case Quad: {
        // Load a 3D model of a cube with 12 triangles
        auto subMeshes = loadMesh(dataDir / "texture_plane.obj");
        std::move(std::begin(subMeshes), std::end(subMeshes), std::back_inserter(scene.meshes));
        scene.lights.emplace_back(PointLight { glm::vec3(-1.0, 1.5, -1.0), glm::vec3(1) });
    } break;
    case BilinearDebug: {
        // Load a 3D model of a cube with 12 triangles
        auto subMeshes = loadMesh(dataDir / "bilinear.obj");
        std::move(std::begin(subMeshes), std::end(subMeshes), std::back_inserter(scene.meshes));
        scene.lights.emplace_back(PointLight { glm::vec3(-1.0, 1.5, -1.0), glm::vec3(1) });
    } break;
    case Transparent: {
        // Load a 3D model of a cube with 12 triangles
        auto subMeshes = loadMesh(dataDir / "transparent.obj");
        std::move(std::begin(subMeshes), std::end(subMeshes), std::back_inserter(scene.meshes));
        scene.lights.emplace_back(PointLight { glm::vec3(-1.0, 1.5, -1.0), glm::vec3(1) });
    } break;
    case CubeTextured: {
    ...
    case Energy: {
        auto subMeshes = loadMesh(dataDir / "energy.obj");
        std::move(std::begin(subMeshes), std::end(subMeshes), std::back_inserter(scene.meshes));
        scene.lights.emplace_back(PointLight { glm::vec3(-1, 1, -1), glm::vec3(1) });
    }
        break;
